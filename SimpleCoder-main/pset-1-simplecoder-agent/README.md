...
Important: run the CLI with the same Python that installed the package. If you use conda or system Python, you must activate the `.venv` first or call the venv Python directly:

```bash
/path/to/pset-1-simplecoder-agent/.venv/bin/python -m simplecoder.main --interactive --verbose
```
...
If you see `ModuleNotFoundError: No module named 'simplecoder'`, you are using a different Python than the one where you installed the package. Activate `.venv` or use its Python directly (see Quickstart).
